public class Trainer {
    ...
    public Network train(int generations, DataSet data){
    ...
    private Network[] generateBatch(int size){
        Network[] res = new Network[size];
        for(int i = 0; i < size; i++){
            res[i] = new Network(copyLayers(base.getLayers()));
            res[i].genNetwork();
        }
        return res;
    }
    private Layer[] copyLayers(Layer[] layers){
        Layer[] res = new Layer[layers.length];
